...
User* KnickKnackerIO::getUserByID(unsigned int id) {
  dirExists();
  User* user = nullptr;
...
std::vector<Comment*> KnickKnackerIO::getCommentsByIID(unsigned int i_id) {
  dirExists();
  ...
  if (file.is_open()) {
    ...
    while (std::getline(file, line)) {
      ...
      if (std::stoi(row[1]) == i_id) {
        comments.push_back(new Comment(
          (unsigned int)std::stoi(row[0]),
